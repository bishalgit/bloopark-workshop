odoo.define('portal_ronba.DealerOrderList', (require) => {
    ...
    const WidgetDealerOrderList = Widget.extend({
        selector: '.dealer_order_list',
        init: function () {
        ...
        start () {
            ...
            var self = this;
            this._rpc({
                route: '/my/dealer_orders/list_json',
                params: {},
            }).then(function (data) {
                ...
                for (const [key, value] of Object.entries(data)) {
                  values.push({
                    ...
                    url: value["url"], // to use in href for detail
                    edit_url: value["edit_url"], // to use in href for edition
                    employee_name: value["employee_name"],
                    action_edit: value["action_edit"]
                  })
                }
                var options = {
                  valueNames: [ 'name', 'employee_name', 'state', 'date_order' ],
                  ...
                  let edit_section = values.action_edit ? `<div class="dealer-order-icon">
                  ...
                  res = `<div class="order-row">
                            <a id=${values.id} href=${values.url}></a>
                                <div class="order-row-data order-list-item list-group-item list-group-item-action">
                                    ...
                                    <div class="order-employee-name employee_name">${values.employee_name}</div>
                                    <div class="order-state state">${values.state}</div>
                                    <div class="dealer-order-date date_order">${values.date_order}</div>
                                    ${edit_section}
                                </div>
                            </a>
                        </div>`;
                ...
                self.order_lists = new List('dealer-orders', options, values);
   ...
   registry.WidgetDealerOrderList = WidgetDealerOrderList;
